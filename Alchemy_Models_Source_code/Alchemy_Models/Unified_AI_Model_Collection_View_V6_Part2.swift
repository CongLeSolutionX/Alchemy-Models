...
// --- Modern WrappingHStack (Corrected) ---
...
struct WrappingHStack<Item:Hashable, Content:View>: View {
    ...
    var body: some View {
        // Use a VStack to allow GeometryReader to propose a flexible height initially
        VStack {
            GeometryReader{ g in
                self.generateContent(in: g)
            }
        }
        .frame(height: totalHeight) // Set the determined height
    }
    ...
    func generateContent(in g: GeometryProxy) -> some View {
        ...
        return ZStack(alignment: .topLeading) {
            ForEach(items, id: \.self) { item in
                ...
                .padding(.bottom, verticalSpacing) // Add vertical spacing below items too
                .alignmentGuide(.leading, computeValue: { d in
                    let itemWidth = d.width // Width including padding
                    if (abs(width - itemWidth) > g.size.width) {
                        width = 0         // Reset width for new line
                        height -= d.height // Move 'y' position down by item height (including vertical padding)
                    }
                    let result = width
                    width -= itemWidth // Consume width
                    return result
                })
                .alignmentGuide(.top, computeValue: { d in
                    let result = height
                    // If it's the last item on a line, move the height baseline down for the next implicitly
                    // This logic is handled by the height adjustment in the .leading guide
                    return result
                })
            }
        }
...
//    // Helper to read the calculated height using PreferenceKey
//    func viewHeightReader(_ binding:Binding<CGFloat>) -> some View {
//        GeometryReader { geo -> Color in // Return Color is a common trick for side effects in GeometryReader
//            // DispatchQueue avoids modifying state during view update cycle warning
//            DispatchQueue.main.async {
//                 // Update the binding only if the height has actually changed
//                 if binding.wrappedValue != geo.size.height {
//                    binding.wrappedValue = geo.size.height
//                 }
//            }
//            // Set the preference key value
//            Color.clear.preference(key: FrameHeightPreferenceKey.self, value: geo.size.height)
//        }
//        // If you were *only* using the preference key approach (not the Binding),
//        // you would use .onPreferenceChange here instead of the DispatchQueue hack.
//        // Since we use a Binding updated asynchronously, the .onPreferenceChange isn't strictly needed *here*,
//        // but the preference key itself *is* needed by the GeometryReader above.
//        // .onPreferenceChange(FrameHeightPreferenceKey.self) { binding.wrappedValue = $0 } // Original logic
//    }
//
//    // CORRECTED PreferenceKey Definition
//    struct FrameHeightPreferenceKey: PreferenceKey {
//        // 1. Explicitly define the Value type
//        typealias Value = CGFloat
//
//        // 2. Make defaultValue static
...
//        }
//
//        // 3. Explicitly type the reduce parameters (good practice)
//        static func reduce(value: inout CGFloat, nextValue: () -> CGFloat) {
//            // Combine values by taking the maximum height encountered
