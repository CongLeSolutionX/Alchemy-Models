...
struct SettingsSheet: View {
    ...
    private func applyChanges() {
           ...
           store.backendType != localBackendType
        {
            ...
            store.apiKey = localApiKey // Update store's AppStorage values
            store.openAIModelName = localOpenAIModelName
            store.openAITemperature = localOpenAITemperature
            store.openAIMaxTokens = localOpenAIMaxTokens
            store.coreMLModelName = localCoreMLModelName
            store.backendType = localBackendType // This setter triggers configureBackend
        }

        // If backend type changed, reconfigure (redundant if setter triggered it, but safe)
         // The store.backendType = localBackendType setter *should* handle this via configureBackend
         // If not implementing that way, manually reconfigure here:
        // if needsBackendUpdate {
        //     store.configureBackend() // Or call the onUpdate closure if configureBackend isn't sufficient
        // }
         
         print("Settings applied. Backend type: \(store.backendType)")
    }
}
