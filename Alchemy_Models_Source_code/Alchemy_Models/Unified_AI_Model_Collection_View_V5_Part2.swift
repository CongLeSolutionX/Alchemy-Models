...
//    }
//    
//    // --- Add Helper to find model by ID for navigation ---
//    func findModel(byId modelId: String) -> (any UnifiedAIModel)? {
//        // Search efficiently if possible (e.g., if models were in a dictionary)
//        // For an array, linear search is okay for moderate numbers
//        return allModels.first { $0.id == modelId }
//    }
//    
...
//                    // *** CHANGE 1: Navigate using model.id ***
...
//            .padding(.vertical, 10)
//        }
//        // *** CHANGE 2: Define navigation destination for String (the ID) ***
//        .navigationDestination(for: String.self) { modelId in
//            // *** CHANGE 3: Look up the model using the ID from the VM ***
//            if let model = vm.findModel(byId: modelId) {
//                ModelDetailUnifiedView(model: model)
//            } else {
//                // Fallback view if the model couldn't be found (should ideally not happen if VM is up-to-date)
//                ContentUnavailableView("Model Not Found", systemImage: "questionmark.diamond", description: Text("The selected model (ID: \(modelId)) could not be loaded."))
//            }
//        }
//        .refreshable {
//            print("Refreshing models for \(vm.selection.rawValue)...")
//            // Make sure loadModels can be called directly or create an async version
//            vm.loadModels(clearExisting: false) // Assuming loadModels is async or wrapped appropriately
//        }
//    }
//    
//    @ViewBuilder
...
//        if let value = value {
//            // Call the other metadataRow, which will also need @ViewBuilder
...
//        // Check for non-empty string correctly
...
//                Text(validValue) // Use the unwrapped, validated value
//                    .font(.callout)
